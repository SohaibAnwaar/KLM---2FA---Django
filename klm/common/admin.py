...
class AutoCompleteAdminMixin:
    ...
    def setup_raw_id_fields(self, model):
        return tuple(f.name for f in model._meta.get_fields() if isinstance(f, models.ManyToManyField))
...
class ModelAdmin(admin.ModelAdmin):
    ...
    def get_list_display(self, request):
        if self.list_display == ("__str__",):
            list_display = [
                field.name for field in self.model._meta.get_fields() if not self.check_related_field(field)
            ]
        ...
        if hasattr(self, "exclude_list_display"):
            list_display = [field for field in list_display if field not in self.exclude_list_display]
